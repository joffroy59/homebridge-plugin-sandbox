...
export class SandboxPlatformAccessory {
  ...
  private createHandlers(motionSensorOneService: Service, motionSensorTwoService: Service) {
    ...
    setInterval(() => {
      ...
      this.platform.log.debug(`Update FakegatoService [${this.accessory.displayName}]`
    ...
    setInterval(() => {
      ...
      this.platform.log.debug(`Update FakegatoService [${this.accessory.displayName}]:`, newTemperature);
      this.fakegatoService.addEntry({
        time: new Date().getTime() / 1000,
        temp: newTemperature,
      });
